...
[![Скачать FoATrainer_V19_1.dll](https://img.shields.io/badge/Download-FoATrainer__V19__1.dll-2ea44f?logo=github)](https://github.com/Fenrisu1ven/FoA_Trainer/releases/download/v2.7.1/FoATrainer_V19_1.dll)
...
**Текущая стабильная версия: V19.1 / 2.7.1**

FoA Trainer — внутриигровой BepInEx-трейнер для **Tainted Grail: The Fall of Avalon**. Он объединяет игровые модификаторы, редактирование персонажа и инвентаря, профили, режим полёта / NoClip, штатное управление временем суток и погодой, расширенный Item Spawner с поиском и настраиваемый ESP в одном интерфейсе на английском и русском языках.
...
> [!NOTE]
> V19.1 развивает подтверждённо рабочую V19 и добавляет штатное управление временем суток. Изменения часов и скорости выполняются через `GameRealTime` без изменения Unity `timeScale`. Совместимость со всеми версиями игры не гарантируется.
...
- Текущее время `HH:mm`, точный ввод, ползунок и быстрые кнопки
- Штатная фиксация и скорость суток от 0x до 10x
- Выбор штатных пресетов погоды с автоматическим и принудительным режимами
- Отображение текущей интенсивности осадков, дождя и снега

### Штатное управление погодой

- Список берётся из реальных пресетов `WeatherController`; выдуманные типы погоды не добавляются
- Пресеты применяются штатным методом игры `SetPreset(int)`
- Выбранная штатная кривая немедленно пересчитывается, в том числе при замороженном или замедленном времени суток
- Принудительный режим удерживает выбранную погоду после смены локации, загрузки или дня
- Автоматический режим прекращает удержание и возвращает управление игре
- Штатные кривые интенсивности, вариации и переходы сохраняются
- Режим и выбор погоды хранятся отдельно в BepInEx Config и не меняют старые профили
- Искусственный ползунок скорости перехода не добавлен: игра не предоставляет для этого поддерживаемого API

### Штатное управление временем суток

- Текущее абсолютное время читается из `GameRealTime.WeatherTime`
- Точное время устанавливается официальным методом `SetWeatherTime(hour, minute)` с сохранением событий времени, дня/ночи, квестов и спавнеров
- Используется штатное правило следующего дня: более раннее время переводит часы через полночь, а не уменьшает игровую дату
- Добавлены ползунок 00:00-23:59, точный ввод `HH:mm` и кнопки «Рассвет», «Утро», «Полдень», «Вечер», «Полночь»
- Время рассвета получается из игрового `ARDateTime.NightEndTime`
- Фиксация и множитель меняют только естественное течение суток через `SetWeatherDayDuration`; Unity `timeScale`, AI и обычный игровой процесс продолжают работать
- Управление временем и Weather Override работают независимо
- Используется мгновенная штатная смена часов; искусственный плавный переход не добавлен, поскольку `GameRealTime` не предоставляет поддерживаемого API перехода
...
ESP — одна из основных функций V19. Оверлей продолжает работать, когда меню трейнера скрыто.
...
V19 сохраняет безопасный для производительности сканер из стабильной ветки V18.4:
...
3. Скачайте `FoATrainer_V19_1.dll` из [последнего релиза GitHub](https://github.com/Fenrisu1ven/FoA_Trainer/releases/latest).
...
Настройки погоды и времени суток намеренно хранятся в существующем BepInEx Config (`BepInEx/config/rijiy.foa.trainer.v19.cfg`), а не в профилях. Старые профили совместимы и не перезаписывают режимы штатного управления погодой/временем.

## Скриншоты

### Штатное управление погодой — V19

![Штатное управление погодой в FoA Trainer V19](screenshots/weather-ru-v19.png)

Панель V19 в реальном времени получает настоящие игровые пресеты и штатную интенсивность осадков. На скриншоте принудительно выбран пресет `Short rain with variation`, а игровая система перешла в состояние сильного дождя.

### Настройки и диагностика — Русский

![Настройки и диагностика FoA Trainer на русском языке](screenshots/settings-ru.png)

### Настройки и диагностика — English

![Настройки и диагностика FoA Trainer на английском языке](screenshots/settings-en.png)

Скриншоты показывают двуязычный интерфейс. В V19 на вкладку **ОПЫТ / ВРЕМЯ** добавлен блок погоды, а скорость движения, высота прыжка и защита от урона при падении сгруппированы во вкладке **ИГРОК**.
...
`tools/build.py` создаёт новый GUID модуля при каждой сборке, поэтому функционально одинаковые DLL могут иметь разные SHA-256. Текущая сборка V19.1 создаётся рядом с сохранёнными бинарниками V19 и V18.4 в `dist/`.
...
- `tools/build.py` — автономный генератор bootstrap/DLL для V19.1
- `dist/FoATrainer_V19_1.dll` — текущий бинарник V19.1
- `dist/FoATrainer_V19.dll` — сохранённый предыдущий бинарник V19
- `dist/FoATrainer_V18_4.dll` — сохранённый предыдущий стабильный бинарник
- `assets/foa-trainer-banner.png` — баннер репозитория FoA Trainer
- `screenshots/weather-ru-v19.png` — штатное управление погодой V19 в игре
- `screenshots/settings-en.png` и `screenshots/settings-ru.png` — скриншоты настроек/диагностики на двух языках
...
V19.1 сохраняет ожидаемую диагностику `Runtime started. Patches: 13, missing: 0`. Управление погодой и временем использует штатные игровые контроллеры и не требует новых Harmony-патчей.
