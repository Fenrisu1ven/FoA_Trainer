...
[![Скачать FoATrainer_V19.dll](https://img.shields.io/badge/Download-FoATrainer__V19.dll-2ea44f?logo=github)](https://github.com/Fenrisu1ven/FoA_Trainer/releases/download/v2.7.0/FoATrainer_V19.dll)
...
**Текущая стабильная версия: V19 / 2.7.0**

FoA Trainer — внутриигровой BepInEx-трейнер для **Tainted Grail: The Fall of Avalon**. Он объединяет игровые модификаторы, редактирование персонажа и инвентаря, профили, режим полёта / NoClip, штатное управление погодой, расширенный Item Spawner с поиском и настраиваемый ESP в одном интерфейсе на английском и русском языках.
...
> [!NOTE]
> V19 развивает подтверждённо рабочую стабильную ветку V18.4. В ней сохранена консервативная архитектура Mono.CSharp и добавлено управление штатными пресетами погоды без замены игровой симуляции. Совместимость со всеми версиями игры не гарантируется.
...
- Скорость игры, заморозка и скорость течения времени суток
...
3. Скачайте `FoATrainer_V19.dll` из [последнего релиза GitHub](https://github.com/Fenrisu1ven/FoA_Trainer/releases/latest).
...
Настройки погоды намеренно хранятся в отдельном BepInEx Config V19 (`BepInEx/config/rijiy.foa.trainer.v19.cfg`), а не в профилях. Старые профили V18.4 совместимы и не перезаписывают автоматический/принудительный режим погоды.
...
`tools/build.py` создаёт новый GUID модуля при каждой сборке, поэтому функционально одинаковые DLL могут иметь разные SHA-256. Текущая сборка V19 создаётся рядом с сохранённым предыдущим бинарником V18.4 в `dist/`.
...
- `tools/build.py` — автономный генератор bootstrap/DLL для V19
- `dist/FoATrainer_V19.dll` — текущий бинарник V19
...
V19 сохраняет ожидаемую диагностику `Runtime started. Patches: 13, missing: 0`. Поддержка погоды работает через штатный контроллер и не требует нового Harmony-патча.
